...
def create_sample_climate_data():
    ...
    base_temp = 15 + 10 * np.sin(
        2 * np.pi * np.arange(240) / (24 * 30)
    )  # Seasonal cycle
    ...
    try:
        ...
        config = ZarrConverterConfig(
            chunking={
                ...
                "lat": 90,
                "lon": 180,
            },  # Reasonable chunking for climate data
        ...
        if has_token:
            ...
            try:
                ...
                if "already has a write session" in error_msg:
                    print(
                        "ℹ️  Datamesh write session already exists for this datasource"
                    )
                    print("This is normal if you've run this demo recently.")
                    print("The data was likely written successfully in a previous run.")
                    print(
                        "To write again, you may need to wait for the session to expire or use a different datasource ID."
                    )
                else:
            ...
            print(
                "# This would write the data to datamesh datasource 'zarrio_demo_climate_data'"
            )
        ...
        if has_token:
            ...
            print(
                f'  --datamesh-datasource \'{{"id":"{datasource_id}","name":"Demo Data","coordinates":{{"x":"lon","y":"lat","t":"time"}}}}\' \\'
            )
        ...
        if has_token:
            ...
            print(
                "ℹ️  Set DATAMESH_TOKEN environment variable to actually write to datamesh"
            )
