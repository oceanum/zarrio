...
class PackingConfig(BaseModel):
    ...
    check_range_exceeded: bool = Field(
        True, description="Whether to check if data exceeds specified ranges"
    )
    ...
    def validate_range_exceeded_action(cls, v: str) -> str:
        if v not in ["warn", "error", "ignore"]:
            raise ValueError(
                "range_exceeded_action must be one of 'warn', 'error', 'ignore'"
            )
        return v
...
class CompressionConfig(BaseModel):
    ...
    method: Optional[str] = Field(
        None, description="Compression method (e.g., 'blosc:zstd:3')"
    )
...
class TimeConfig(BaseModel):
    ...
    global_start: Optional[Union[str, datetime]] = Field(
        None, description="Global start time"
    )
    global_end: Optional[Union[str, datetime]] = Field(
        None, description="Global end time"
    )
...
class MissingDataConfig(BaseModel):
    ...
    retries_on_missing: int = Field(
        0, description="Number of retries if missing values are encountered", ge=0
    )
    ...
    @classmethod
    def validate_missing_check_vars(
        cls, v: Optional[Union[str, List[str]]]
    ) -> Optional[Union[str, List[str]]]:
        if v is not None and not isinstance(v, (str, list)):
            raise ValueError(
                "`missing_check_vars` must be one of 'all', None or a list of data vars"
            )
        if isinstance(v, str) and v != "all":
...
class DatameshConfig(BaseModel):
    ...
    service: str = Field(
        "https://datamesh-v1.oceanum.io", description="Datamesh service URL"
    )
    use_zarr_client: bool = Field(
        True, description="Whether to use the datamesh zarr client for writing"
    )
...
class ZarrConverterConfig(BaseModel):
    ...
    compression: Optional[CompressionConfig] = Field(
        None, description="Compression configuration"
    )
    packing: PackingConfig = Field(
        default_factory=PackingConfig, description="Packing configuration"
    )
    time: TimeConfig = Field(
        default_factory=TimeConfig, description="Time configuration"
    )
    variables: VariableConfig = Field(
        default_factory=VariableConfig, description="Variable configuration"
    )
    missing_data: MissingDataConfig = Field(
        default_factory=MissingDataConfig, description="Missing data configuration"
    )
    ...
    target_chunk_size_mb: Optional[int] = Field(
        None, description="Target chunk size in MB for intelligent chunking"
    )
    ...
    def validate_access_pattern(cls, v: str) -> str:
        if v not in ["temporal", "spatial", "balanced"]:
            raise ValueError(
                "access_pattern must be one of 'temporal', 'spatial', or 'balanced'"
            )
        return v
